...
const dataPoints = [1, 5, 6, 4, 7, 5, 6, 7, 8, 4, 5, 6];
...
class LineChart {
  ...
  init() {
    ...
    const scaleX = (this.scaleX = d3.time.scale().range([0, innerWidth]));

    const scaleY = (this.scaleY = d3.scale.linear().range([innerHeight, 0]));
    ...
    svg.append('path').attr('class', 'chart__line');
...
const lineChart = new LineChart('.vr-yearly-summary-line-chart');

lineChart.render(dataPoints);
